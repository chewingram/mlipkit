...
class MlipModel(ABC):
    ...
    def __init__(self, 
        ...
        self.tmp_dir = self.root_dir.joinpath('mlipkit_tmp') # we don't create it yet. Everytime it is used, it must be created
        if pre_trained == False:
            self.root_dir.mkdir(exist_ok=True, parents=True)
        self.training_dir = self.root_dir.joinpath('training') 
        self.trained_pot_dir = self.root_dir.joinpath('trained_pot')
    ...
    def _set_pretrained_model(self, pre_trained_pot_filepaths):
        ...
        for k in pre_trained_pot_filepaths.keys():
            if k not in self.trained_pot_files.keys():
        ...
        # the files will be copied into the proper trained_pot directory with the proper name and
        # self.trained_pot_filepaths will be updated
        self.trained_pot_filepaths = pre_trained_pot_filepaths
    ...
    def _check_trained_pot_files(self):
        ...
        Returns:
            bool: True if all trained potential files are present; False otherwise.
        """
        bools = [fpath.is_file() for fpath in self.trained_pot_filepaths.values()]
        if all(bools):
            return True
        else:
            return False
    ...
    def _from_json(cls, json_path, train_set=None, ignore_metadata=False):
        ...
        if 'class' not in data.keys():
            ...
            new_obj.root_dir = json_path.parent
            ...
            if new_obj.is_trained == False:
                ...
                if not new_obj._check_trained_pot_files():
                    ...
                    hash_new_trained_pot = new_obj._make_hash_for_trained_pot(list(new_obj.trained_pot_filepaths.values()))
                    ...
                    if ignore_metadata == True:
                        if new_train_set is not None:
                            ...
                            metadata_path = new_obj.trained_pot_dir.joinpath('training.metadata') 
                            ...
                            if hashes_from_metadata is not None:
                                if hash_new_trained_pot == hashes_from_metadata['trained_pot']:
                                    if new_train_set is not None:
                                        ...
                                        if hash_new_train_set != hashes_from_metadata['train_set']:
                                            raise ValueError('The train set you provided is incompatible to the metadata file! If you wish to ignore this, load the model with `ignore_metadata` = True.')
                                        else:
                                            new_obj.train_set = new_train_set
                                            new_obj.hash_train_set = hash_new_train_set
                                            #new_obj._make_metadata() # no need, right?
                                    else:
    ...
    def _make_metadata(self, path=None, hash_trained_pot=None, hash_train_set=None):
        ...
        if path is None:
            path = self.trained_pot_dir.joinpath('training.metadata')
    ...
    def _get_hashes_from_metadata(self):
        ...
        """
        path = self.trained_pot_dir.joinpath('training.metadata') # cannot use self.metadata_path, because sometimes this function is called before setting it
    ...
    def _store_trained_model(self, lock=True, save_train_set=False, train_set_saving_loc="./"):
        ...
        if self.trained_pot_dir.is_dir():
            shutil.rmtree(self.trained_pot_dir.absolute())
        self.trained_pot_dir.mkdir(parents=True)

        # save trained potential files 
        for v in self.trained_pot_files.values():
            src = self.training_dir.joinpath(v) 
            dst = self.trained_pot_dir.joinpath(v)
        ...
        # generate hashes
        hash_trained_pot = self._make_hash_for_trained_pot(list(self.trained_pot_filepaths.values()))
        hash_train_set = self._make_hash_for_train_set(self.train_set)

        self._make_metadata(self.trained_pot_dir.joinpath('training.metadata'), hash_trained_pot, hash_train_set)
    ...
    def _store_pretrained_model(self, lock=True, save_train_set=False, train_set_saving_loc='./'):
        ...
        for key, value in self.trained_pot_filepaths.items(): 
            src = Path(value)
            proper_filename = self.trained_pot_files[key]
            dst = self.trained_pot_dir.joinpath(proper_filename)
            shutil.copy(src, dst) # copy the trained pot files in the trained pot dir
            self.trained_pot_filepaths[key] = dst.resolve() # update with the proper filepaths
        ...
        if self.train_set is not None:
            ...
            print('The trained model was stored, but no metadata was created since there was no train set.')
    ...
    def train_model(self, train_set=None, training_hyperparameters=None, save_train_set=False, train_set_saving_loc="./"):
        ...
        if self.training_dir.is_dir():
            shutil.rmtree(self.training_dir.absolute())
       ...
       # we need to check if all the mandatory hyperparameters are there (we already checked that no extra, unkown parameters were given)
        ...
        self.trained_pot_dir.mkdir(parents=True, exist_ok=True)
        # run the training
        try:
    ...
    def _general_compute_properties(self, atoms, wdir, parameters=None, mute=True, function=None, update_parameters=False):
        ...
        if parameters is None:
            ...
            to_pass = self.parameters_compute_properties
        else:
    ...
    def compute_properties(self, atoms, wdir, parameters=None, mute=True):
        ...
        if isinstance (atoms, Atoms):
            compute_properties_single_structure(self, atoms, wdir, parameters=parameters, mute=mute)
        else:
