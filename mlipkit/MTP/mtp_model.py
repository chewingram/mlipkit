...
class MTPModel(MlipModel):
    ...
    def _train(self):
        ...
        with open(self.training_dir.joinpath('log_train'), 'r') as fl:
            lines = fl.readlines()
    ...
    def _compute_properties_single(self, atoms, wdir, parameters):
        ...
        wdir.mkdir(exist_ok=True, parents=True)
        pot_path = self.trained_pot_filepaths['potential_file'].resolve()
        self.make_ini_for_lammps(pot_file_path=pot_path, out_file_path=wdir.joinpath('mlip.ini'))
    ...
    def make_ase_calculator(self, parameters=None):
        ...
        pot_file_path = self.trained_pot_filepaths['potential_file']
        ...
        print(f"wdir = {str(self.tmp_dir.resolve())}")
        lammps_calc = LAMMPS(pair_style=f"mlip {out_file_path.resolve()}",
                             pair_coeff=['* *'],
                             tmp_dir=str(self.tmp_dir.resolve()),
