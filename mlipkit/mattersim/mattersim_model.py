...
class MatterSimModel(MlipModel):
    ...
    def _train(self):
        ...
        trained_model_path = finetune_pot(wdir=self.training_dir,
                                          train_set = tmp_dataset[:-n_validation_confs],
                                          test_set = tmp_dataset[-n_validation_confs:],
                                          final_evaluation=False,
                                          eval_dir=None,
                                          saving_path=self.training_dir,
                                          **args_to_pass)
        # after this, we take the best model and save it inside the training_directory
        src_pot = trained_model_path.resolve()
        target_pot = self.training_dir.joinpath(self.trained_pot_files['potential_file']).resolve() # save it inside training_dir; it will be copied into trained_pot_dir later
        ...
        if self.hyperparameters['clean_after_training'] == True:
            [x.unlink(missing_ok=True) for x in self.training_dir.glob('ckpt*')]
            self.training_dir.joinpath('last_model.pth').unlink(missing_ok=True)
            self.training_dir.joinpath('Train_set.traj').unlink(missing_ok=True)
            self.training_dir.joinpath('Test_set.traj').unlink(missing_ok=True)
            
        # must return True for success and False otherwise
        if self.training_dir.joinpath('best_model.pth').is_file():
            success = True
        else:
            success = False
        ...
        if success == True:
            if self.hyperparameters['set_trained_pot_as_new_starting_point'] == True:
                self.hyperparameters['load_model_path'] = self.trained_pot_dir.joinpath(self.trained_pot_files['potential_file']).resolve()
        return success
    ...
    def _compute_properties_single(self, atoms, wdir, parameters):
        ...
        pot_path = self.trained_pot_filepaths['potential_file'].resolve()
    ...
    def make_ase_calculator(self, parameters=None):
        ...
        if 'device' in self.parameters_compute_properties.keys():
            ...
            device = 'cpu'
        load_path=str(self.trained_pot_filepaths['potential_file'])
